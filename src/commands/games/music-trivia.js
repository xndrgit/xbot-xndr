...
module.exports = async (client, interaction, args) => {
    ...
    if (!number || isNaN(number)) return client.errUsage({
        usage: "music-trivia [amount of numbers]",
        type: 'editreply'
    }, interaction);
    ...
    if (interaction.client.playerManager.get(interaction.guild.id)) return client.errNormal({
        error: `You can't use this while a track is playing!`,
        type: 'editreply'
    }, interaction);

    if (interaction.client.triviaManager.get(interaction.guildId)) return client.errNormal({
        error: `There is already a trivia in play!`,
        type: 'editreply'
    }, interaction);
